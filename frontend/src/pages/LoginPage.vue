<template>
  <main class="min-h-screen bg-neon-bg flex items-center justify-center px-4">
    <div class="w-full max-w-md">
      <form
        class="bg-neon-card p-8 rounded-2xl shadow-lg border border-neon-line"
        @submit.prevent="handleLogin"
      >
        <h1 class="text-3xl font-black font-display text-white mb-2 text-center uppercase tracking-tight">
          Entrar
        </h1>
        <p class="text-sm text-slate-400 text-center mb-6">
          Faça login para comprar e acompanhar seus pedidos
        </p>

        <div class="space-y-5">
          <div>
            <label for="login-email" class="block text-sm font-medium text-slate-300 mb-1.5">
              Email
            </label>
            <input
              id="login-email"
              v-model.trim="form.email"
              type="email"
              autocomplete="email"
              required
              class="w-full px-4 py-2.5 bg-neon-card2 border border-neon-line rounded-xl focus:outline-none focus:ring-2 focus:ring-neon-lime text-white placeholder-slate-500"
              placeholder="seu@email.com"
            />
          </div>

          <div>
            <label for="login-password" class="block text-sm font-medium text-slate-300 mb-1.5">
              Senha
            </label>
            <input
              id="login-password"
              v-model="form.password"
              type="password"
              autocomplete="current-password"
              required
              class="w-full px-4 py-2.5 bg-neon-card2 border border-neon-line rounded-xl focus:outline-none focus:ring-2 focus:ring-neon-lime text-white placeholder-slate-500"
              placeholder="********"
            />
          </div>

          <p
            v-if="errorMessage"
            role="alert"
            class="text-sm text-red-400 bg-red-950/40 border border-red-900 rounded-xl px-4 py-2.5"
          >
            {{ errorMessage }}
          </p>

          <button
            type="submit"
            :disabled="authStore.isLoading"
            class="w-full py-3 bg-neon-lime hover:brightness-110 disabled:opacity-60 disabled:cursor-not-allowed text-black font-bold rounded-xl transition-all"
          >
            {{ authStore.isLoading ? 'Entrando...' : 'Entrar' }}
          </button>

          <!-- Divider -->
          <div class="flex items-center gap-3 my-2">
            <span class="flex-1 h-px bg-neon-line"></span>
            <span class="text-xs text-slate-500 font-medium uppercase">ou</span>
            <span class="flex-1 h-px bg-neon-line"></span>
          </div>

          <!-- Google Login Button -->
          <button
            type="button"
            @click="handleGoogleLogin"
            :disabled="googleLoading"
            class="w-full py-3 px-4 rounded-xl font-bold transition-all flex items-center justify-center gap-3 border border-neon-line bg-neon-card2 text-white hover:bg-white/10"
          >
            <svg class="w-5 h-5" viewBox="0 0 24 24">
              <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"/>
              <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
              <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
              <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
            </svg>
            {{ googleLoading ? 'Entrando...' : 'Entrar com Google' }}
          </button>
        </div>

        <p class="mt-6 text-center text-sm text-slate-400">
          Ainda não tem conta?
          <router-link to="/register" class="text-neon-lime hover:underline font-semibold">
            Criar Conta
          </router-link>
        </p>
      </form>
    </div>
  </main>
</template>

<script setup>
import { ref } from 'vue';
import { useRoute, useRouter } from 'vue-router';
import { useAuthStore } from '../stores/auth';

const router = useRouter();
const route = useRoute();
const authStore = useAuthStore();

const form = ref({
  email: '',
  password: '',
});

const errorMessage = ref('');
const googleLoading = ref(false);

async function handleLogin() {
  errorMessage.value = '';

  try {
    const user = await authStore.login(form.value.email, form.value.password);
    form.value.password = '';

    const redirect = typeof route.query.redirect === 'string' ? route.query.redirect : null;
    if (redirect) {
      await router.replace(redirect);
    } else {
      await router.replace(user?.role === 'admin' ? '/admin' : '/');
    }
  } catch (err) {
    // Keep the message generic so the form never confirms whether an email exists.
    errorMessage.value = err?.message || 'Nao foi possivel entrar. Verifique email e senha.';
  }
}

async function handleGoogleLogin() {
  googleLoading.value = true;
  errorMessage.value = '';

  try {
    if (typeof window.google?.accounts?.id !== 'undefined') {
      // Google Identity Services loaded — request the token
      window.google.accounts.id.initialize({
        client_id: import.meta.env.VITE_GOOGLE_CLIENT_ID,
        callback: async (response) => {
          if (response.credential) {
            await authStore.loginWithGoogle(response.credential);
            const redirect = typeof route.query.redirect === 'string' ? route.query.redirect : null;
            await router.replace(redirect || '/');
          }
          googleLoading.value = false;
        },
        cancel_on_tap_outside: false,
      });
      window.google.accounts.id.prompt();
    } else {
      // GIS not loaded — redirect to the backend Google auth URL
      // (fallback: use the popup approach)
      errorMessage.value = 'O login com Google está disponível apenas em navegadores modernos. Use email e senha por enquanto.';
      googleLoading.value = false;
    }
  } catch (err) {
    errorMessage.value = err?.message || 'Falha no login com Google.';
    googleLoading.value = false;
  }
}
</script>